...
const linterSubSchema = new mongoose.Schema({
  score: Number, maxScore: Number, errors: [String], warnings: [String], passed: Boolean
}, { _id: false, suppressReservedKeysWarning: true });

const fnTestSchema    = new mongoose.Schema({}, { strict: false, _id: false });
const intTestSchema   = new mongoose.Schema({ name: String, passed: Boolean, weight: Number, earned: Number }, { _id: false });

const linterBucketSchema = new mongoose.Schema({
  score: Number, maxScore: Number,
  htmlhint: linterSubSchema, stylelint: linterSubSchema, eslint: linterSubSchema
}, { _id: false });

const functionalityBucketSchema = new mongoose.Schema({
  score: Number, maxScore: Number, earned: Number, rawMax: Number, tests: [fnTestSchema]
}, { _id: false });

const interactionBucketSchema = new mongoose.Schema({
  score: Number, maxScore: Number, tests: [intTestSchema]
}, { _id: false });

const visualBucketSchema = new mongoose.Schema({
  score: Number, maxScore: Number, diffPercent: Number,
...
export const evaluationRunSchema = new mongoose.Schema({
